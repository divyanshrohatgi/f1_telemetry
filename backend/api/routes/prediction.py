"""
ML prediction routes for tyre degradation and pit window recommendations.
"""
...
@router.post("/v1/predict/degradation", response_model=DegradationResponse)
async def predict_degradation(request: DegradationRequest):
    """
    Predict tyre degradation curve for a given compound and conditions.
    ...
    Falls back to a linear estimate if the model is not trained yet.
    """
    try:
    ...
    # Find cliff lap (largest acceleration in degradation)
...
@router.post("/v1/predict/pit-window", response_model=PitWindowResponse)
async def predict_pit_window_route(request: PitWindowRequest):
    """
    Recommend optimal pit window based on current tyre state and conditions.
    """
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Pit window prediction failed: {exc}")
...
def _find_cliff_lap(curve: list[dict]) -> Optional[int]:
    ...
    # Second derivative: where the slope increases most
    second_deriv = []
    ...
    # Find maximum positive curvature (cliff)
    cliff_idx, cliff_d2 = max(second_deriv, key=lambda x: x[1])

    # Only report a cliff if the acceleration is significant
    if cliff_d2 > 0.03:
